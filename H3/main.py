...
def main():
    ...
    population = [Forager(random.randrange(config.WORLD_WIDTH),
    ...
    with open(os.path.join(config.LOG_DIR, "stats_log.csv"),  "w", newline="") as fcsv, \
        ...
        stats_writer = csv.DictWriter(
            ...
            fieldnames=["generation", "best_fitness", "avg_fitness",
                        "median_eta", "median_lambda", "median_drift"]
        )
        shift_writer = csv.DictWriter(
            fshift,
            fieldnames=["shift_id", "generation", "best_fitness", "avg_fitness"]
        )
        ...
        for gen in range(1, config.NUM_GENERATIONS + 1):
            ...
            stats_writer.writerow({
                ...
                "median_eta"   : stats["median_eta"],
                "median_lambda": stats["median_lambda"],
                "median_drift" : stats["median_drift"]
            })

            print(
                f"Gen {gen:4d} | best {stats['best_fitness']:.2f} "
                f"| avg {stats['avg_fitness']:.2f} "
                f"| η̃ {stats['median_eta']:.4f} "
                f"| λ̃ {stats['median_lambda']:.4f} "
                f"| Δθ̃ {stats['median_drift']:.4f}"
            )

            if did_shift:
                print(f"[ENV SHIFT] at Gen {gen}")
                shift_mode, postshift_counter, shift_id = True, 0, shift_id + 1
            ...
            if gen % config.SAVE_INTERVAL == 0 or gen == config.NUM_GENERATIONS:
                for i, ag in enumerate(population):
                    np.save(
                        os.path.join(config.GENOME_DIR, f"gen{gen:04d}_ind{i:03d}.npy"),
                        ag.genome
                    )
