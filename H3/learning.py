...
CLIP_VAL   = 5.0          # keep live weights bounded
CLIP_DRIFT = 1_000_000.0  # any larger drift is treated as invalid
...
class LearningModule:
    ...
    def learn(self, creature, prev_obs, action, reward, new_obs):
        ...
        q_prev.backward()
        grads = torch.cat(
            [p.grad.view(-1) for p in creature.brain.parameters()]
        ).numpy()

        creature.e_trace = (
            self.gamma * creature.lmbda * creature.e_trace + grads
        )

        delta = (
            creature.eta * td_err * creature.e_trace
            if creature.hebb == 0
            else creature.eta * grads               # Hebbian
        )

        # --------- apply ONLY to live weights (no genomic change) -----------
        i = 0
        sz_w1 = config.HIDDEN_SIZE * config.NUM_SENSORS
        d_w1  = delta[i:i+sz_w1].reshape(config.HIDDEN_SIZE, config.NUM_SENSORS); i += sz_w1
        d_b1  = delta[i:i+config.HIDDEN_SIZE];                                      i += config.HIDDEN_SIZE
        sz_w2 = config.NUM_OUTPUTS * config.HIDDEN_SIZE
        d_w2  = delta[i:i+sz_w2].reshape(config.NUM_OUTPUTS, config.HIDDEN_SIZE);   i += sz_w2
