...
class Brain(nn.Module):
    def __init__(self, genome: np.ndarray):
        ...
        i = 0
        size_w1 = config.HIDDEN_SIZE * config.NUM_SENSORS
        W1 = genome[i:i+size_w1].reshape(config.HIDDEN_SIZE, config.NUM_SENSORS)
        i += size_w1

        b1 = genome[i:i+config.HIDDEN_SIZE]
        i += config.HIDDEN_SIZE

        size_w2 = config.NUM_OUTPUTS * config.HIDDEN_SIZE
        W2 = genome[i:i+size_w2].reshape(config.NUM_OUTPUTS, config.HIDDEN_SIZE)
        i += size_w2
